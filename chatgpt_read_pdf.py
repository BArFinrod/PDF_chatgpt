...
st.markdown('1. Obtenga un token de forma gratuita en la siguiente dirección: [OpenIA](https://platform.openai.com/account/billing/overview) . Es posible que luego de usarlo varias veces en esta aplicación, el token quede obsoleto \
...
siguiendo los pasos del video de YouTube indicado anteriormente', unsafe_allow_html=True)
st.markdown('2. Ingrese el token en la siguiente espacio indicado. Esta aplicación no almacena tokens ni ningún tipo de información: De hecho no tengo espacio para ello', unsafe_allow_html=True)
st.markdown('3. Suba su archivo PDF y comience a hacer preguntas sobre el mismo.', unsafe_allow_html=True)
